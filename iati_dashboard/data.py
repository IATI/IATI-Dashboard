...
def get_registry_id_matches():
    ...
    with open(filepaths.join_base_path("registry_id_relationships.csv")) as f:
        ...
        registry_matches = {row["previous_registry_id"]: row["current_registry_id"] for row in reader}
...
def create_codelist_mapping(major_version):
    codelist_mapping = {}
    for x in json.load(
        open(filepaths.join_data_path("IATI-Codelists-{}/out/clv2/mapping.json".format(major_version)))
    ):
        if "condition" in x:
            pref, attr = x["path"].rsplit("/", 1)
            path = "{0}[{1}]/{2}".format(pref, x["condition"], attr)
        else:
            path = x["path"]
...
codelist_sets = {
    major_version: {
        cname: set(c["code"] for c in codelist["data"])
        for cname, codelist in JSONDir(
            filepaths.join_data_path("IATI-Codelists-{}/out/clv2/json/en/".format(major_version))
        ).items()
    }
...
codelist_lookup = {
    major_version: {
        cname: {c["code"]: c for c in codelist["data"]}
        for cname, codelist in JSONDir(
            filepaths.join_data_path("IATI-Codelists-{}/out/clv2/json/en/".format(major_version))
        ).items()
    }
...
secondary_publishers = [
    publisher
    for publisher, stats in JSONDir(filepaths.join_stats_path("current/aggregated-publisher")).items()
...
def element_slug(element):
    return re.sub(r"{[^}]*}", "", element.replace("{http://www.w3.org/XML/1998/namespace}", "xml:").replace("/", "_"))
...
slugs = {
    ...
    "org_type": make_slugs(
        ["accountable_org", "extending_org", "funding_org", "implementing_org", "provider_org", "receiver_org"]
    ),
}
