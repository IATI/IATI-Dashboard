...
columns = {
    "summary": [
        ...
        (
            ...
            "Weighted Average",
            0,
        ),  # i.e. don't include the average within the calculation of the average
    ],
    "core": [
        ("version", "Version", 1),
        ...
        budgets = max(
            ...
            key=(
                lambda x: (
                    ...
                    if stats["by_hierarchy"][x]["comprehensiveness_denominator_default"] > 0
                    else -1
                )
            ),
        )
...
def get_first_hierarchy_with_commitments(stats):
    ...
    """
    hierarchies_with_commitments = {
        x: y["comprehensiveness"]["transaction_commitment"]
...
def generate_row(publisher):
    ...
    for slug in column_slugs["core"] + column_slugs["financials"] + column_slugs["valueadded"]:
        ...
        if slug == "budget":
            ...
            budget_not_provided_valid = publisher_base.get("comprehensiveness_with_validation", {}).get(
