...
organisation_type_dict = codelist_dict(
    filepaths.join_data_path("IATI-Codelists-2/out/clv2/json/en/OrganisationType.json")
)
country_dict = codelist_dict(filepaths.join_data_path("IATI-Codelists-2/out/clv2/json/en/Country.json"))
region_dict = codelist_dict(filepaths.join_data_path("IATI-Codelists-2/out/clv2/json/en/Region.json"))

aggregated_publisher = data.JSONDir(filepaths.join_stats_path("current/aggregated-publisher/"))
...
for publisher, publisher_data in aggregated_publisher.items():
    if publisher in data.ckan_publishers:
        ...
        if publisher_country_code in country_dict or publisher_country_code in region_dict:
            publishers_by["country"][
                country_dict.get(publisher_country_code) or region_dict.get(publisher_country_code)
            ] += 1
        else:
...
publishers_quarterly = []
publishers_by_date = json.load(open(filepaths.join_stats_path("gitaggregate-dated/publishers.json")))
...
with open(filepaths.join_out_path("speakers_kit.csv"), "w") as fp:
    writer = csv.DictWriter(fp, fieldnames)
    writer.writeheader()
    for (
        publishers_by_type,
        publishers_by_country,
        publishers_quarterly_,
        activities_by_country,
        activities_by_region,
    ) in zip_longest(
