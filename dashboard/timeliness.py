...
def publisher_frequency_sorted():
    return sorted(publisher_frequency(), key=lambda tup: (frequency_index(tup[3]), tup[1]))
...
def publisher_timelag():
    return [
        ...
        for publisher, agg in JSONDir(filepaths.join_stats_path("current/aggregated-publisher")).items()
    ]


def publisher_timelag_sorted():
    return sorted(publisher_timelag(), key=lambda tup: (timelag_index(tup[3]), tup[1]))
