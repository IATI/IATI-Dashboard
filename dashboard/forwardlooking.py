            ...
            if len(hierarchies_with_nonzero_budgets) != 1:
                ...
                if row["budget_not_provided"]:
                    row["year_columns"][1][year] += (
                        publisher_stats["forwardlooking_activities_with_budget_not_provided"].get(year) or 0
                    )
            else:
                # Else, use the hierarchy which they are reported at
                row["year_columns"][0][year] = (
                    by_hierarchy[hierarchies_with_nonzero_budgets[0]]["forwardlooking_activities_current"].get(year)
                    or 0
                )
                row["year_columns"][1][year] = (
                    by_hierarchy[hierarchies_with_nonzero_budgets[0]]["forwardlooking_activities_with_budgets"].get(
                        year
                    )
                    or 0
                )
                if row["budget_not_provided"]:
                    row["year_columns"][1][year] += (
                        by_hierarchy[hierarchies_with_nonzero_budgets[0]][
