...
app.jinja_env.globals["commit_hash"] = (
    subprocess.run("git show --format=%H --no-patch".split(), capture_output=True).stdout.decode().strip()
)
app.jinja_env.globals["stats_commit_hash"] = (
    subprocess.run("git -C stats-calculated show --format=%H --no-patch".split(), capture_output=True)
    .stdout.decode()
    .strip()
)
app.jinja_env.globals["stats_url"] = "https://stats.codeforiati.org"
app.jinja_env.globals["stats_gh_url"] = (
    "https://github.com/codeforIATI/IATI-Stats-public/tree/" + app.jinja_env.globals["stats_commit_hash"]
)
...
def org_type(slug):
    ...
    return render_template("org_type.html", slug=slug, page="org_ids")
...
@app.route(
    '/<any("img/tablesorter-icons.gif", "img/favicon.ico", "img/favicon-16x16.png", "img/favicon-32x32.png"):filename>'
)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--live", action="store_true", help="Run a development server")
        ...
        def url_generator():
            ...
            for major_version, codelist_slugs in slugs["codelist"].items():
                for slug in codelist_slugs["by_slug"]:
                    yield "codelist", {"slug": slug, "major_version": major_version}
