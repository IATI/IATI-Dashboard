...
def generate_row(publisher):
    ...
    data_2015 = publisher_stats["reference_spend_data_usd"].get(
        "2015", {"ref_spend": "", "official_forecast": "", "not_in_sheet": True}
    )

    # Compute reference data as $USDm
    row["reference_spend_2014"] = (
        round((float(data_2014["ref_spend"]) / 1000000), 2) if is_number(data_2014["ref_spend"]) else "-"
    )
    row["reference_spend_2015"] = (
        round((float(data_2015["ref_spend"]) / 1000000), 2) if is_number(data_2015["ref_spend"]) else "-"
    )
    row["official_forecast_2015"] = (
        round((float(data_2015["official_forecast"]) / 1000000), 2)
        if is_number(data_2015["official_forecast"])
        else "-"
    )
    ...
    spend_ratio_candidates = [
        (
            (row["iati_spend_2014"] / row["reference_spend_2014"])
            if (row["reference_spend_2014"] > 0) and is_number(row["reference_spend_2014"])
            else 0
        ),
        (
            (row["iati_spend_2015"] / row["reference_spend_2015"])
            if (row["reference_spend_2015"] > 0) and is_number(row["reference_spend_2015"])
            else 0
        ),
        (
            (row["iati_spend_2015"] / row["official_forecast_2015"])
            if (row["official_forecast_2015"] > 0) and is_number(row["official_forecast_2015"])
            else 0
        ),
    ]
    ...
    if (
        (row["iati_spend_2014"] == 0 or row["reference_spend_2014"] == "-")
